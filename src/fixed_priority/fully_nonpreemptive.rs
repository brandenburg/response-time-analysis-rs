...
pub struct TaskUnderAnalysis<'a, AB: ArrivalBound + ?Sized> {
    ...
    /// to the maximum WCET of any lower-priority task.
    pub blocking_bound: Service,
}

/// Try to find a response-time bound for a task under
/// fully-nonpreemptive fixed-priority scheduling on a dedicated
/// uniprocessor.
///
...
/// See also [the Coq-verified instantiation](http://prosa.mpi-sws.org/branches/master/pretty/prosa.results.fixed_priority.rta.fully_nonpreemptive.html).
    ...
    AB: ArrivalBound + ?Sized,
{
    ...
    // In the fully non-preemptive model, no job can be preempted
    // prior to its completion. In other words, once a job starts
    // running, it is guaranteed to finish. Thus, we can set the
    // task-level run-to-completion threshold to epsilon.
